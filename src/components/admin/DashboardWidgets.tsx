...
function PremiumCard({ children, className = '' }: { children: React.ReactNode; className?: string }) {
    return (
        <div className={`card-depth relative ${className}`}>
            {children}
        </div>
    );
}
    ...
    return (
        <PremiumCard className="p-8 relative group">
            <div className="flex justify-between items-start mb-6">
                <div className="w-12 h-12 rounded-2xl bg-[#f0f9ff] text-[#0066CC] flex items-center justify-center shadow-inner">
                    <IndianRupee size={24} strokeWidth={2} />
                </div>
                <span className="flex items-center gap-1 text-xs font-bold bg-[#f0f9ff] text-[#0066CC] px-2.5 py-1 rounded-full border border-[#A7D3E0]">
                    <TrendingUp size={12} />
                    +12.5%
                </span>
            </div>
            ...
            <div className="relative pt-2">
                ...
                <div className="h-3 w-full bg-slate-100 rounded-full overflow-hidden">
                    <div
                        className="h-full bg-[#0066CC] rounded-full relative overflow-hidden"
            ...
            <div className="p-8 border-b border-slate-50 flex justify-between items-center">
                <div>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2">
                        <Package2 size={20} className="text-[#663399]" />
                        Material Inventory
                    </h3>
                    <p className="text-sm text-slate-400 mt-1">Stock levels vs Consumption</p>
                </div>
                <button className="text-xs font-bold text-[#663399] bg-[#faf5ff] hover:bg-[#A7D3E0] px-3 py-1.5 rounded-lg transition-colors">
                    View Full Report
                </button>
            </div>
                    ...
                    <div key={idx} className="group">
                        <div className="flex justify-between items-end mb-2">
                            ...
                            <div className="text-right">
                                <span className={`text-xs font-bold px-2 py-0.5 rounded-md ${item.status === 'Critical' ? 'bg-[#fffbef] text-[#b45309]' : 'bg-[#f0fdf4] text-[#15803d]'
                                    }`}>
                                    {item.balance} Left
                                </span>
                            </div>
